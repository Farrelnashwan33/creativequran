...
export default function ArtikelPage() {
  return (
    <main className="min-h-screen bg-white pb-20">
      <div className="bg-secondary/30 pt-20 pb-32 px-6">
        <div className="container mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Artikel & Panduan</h1>
          <p className="text-foreground/60 text-lg max-w-2xl">Perdalam wawasan keislaman Anda melalui artikel pilihan yang bersumber dari dalil yang shahih.</p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-6 -mt-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((art) => (
            <div key={art.title} className="bg-white rounded-[32px] overflow-hidden soft-shadow border border-secondary group hover:border-primary/20 transition-all">
              <div className={`h-48 ${art.bgColor} flex items-center justify-center relative overflow-hidden`}>
                <art.icon size={80} className={`${art.color} opacity-20 scale-150 absolute`} />
                <art.icon size={64} className={`${art.color} relative z-10`} />
              </div>
              <div className="p-8">
                <span className={`text-[10px] font-bold uppercase tracking-widest ${art.color} bg-white px-3 py-1 rounded-full soft-shadow inline-block mb-4`}>
                  {art.category}
                </span>
                <h3 className="text-xl font-bold mb-3 group-hover:text-primary transition-colors">{art.title}</h3>
                <p className="text-foreground/60 text-sm leading-relaxed mb-6">{art.excerpt}</p>
