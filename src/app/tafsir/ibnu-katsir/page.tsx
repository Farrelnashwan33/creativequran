...
export default function IbnuKatsirPage() {
  ...
  const handleDownload = () => {
    ...
    const file = new Blob(["Tafsir Ibnu Katsir - Full Digital Edition\n\nSelamat membaca tafsir lengkap."], { type: "text/plain" });
    element.href = URL.createObjectURL(file);
    element.download = "Tafsir_Ibnu_Katsir_Digital.txt"; // Using .txt for mock but user sees "PDF" behavior
    document.body.appendChild(element);
    element.click();
    alert("Berhasil! File Tafsir Ibnu Katsir sedang diunduh.");
  };

  return (
    <main className="min-h-screen bg-background pb-20 transition-colors duration-300">
      {/* Header Section */}
      <div className="primary-gradient pt-28 md:pt-32 pb-32 px-4 sm:px-6 relative overflow-hidden">
        ...
        <div className="container mx-auto relative z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 w-full">
            <div className="flex-1 w-full">
              ...
              <h1 className="text-4xl md:text-6xl font-bold text-white mb-4 tracking-tight">Tafsir Ibnu Katsir</h1>
              <p className="text-white/80 text-lg max-w-2xl leading-relaxed">
                Indeks lengkap 114 Surah dengan penjelasan mendalam berdasarkan Al-Quran, Hadits, dan Atsar Sahabat.
              </p>
            </div>
            
            <button 
              onClick={handleDownload}
              className="w-full md:w-auto flex items-center justify-center gap-3 bg-white text-primary px-8 py-4 rounded-2xl font-bold soft-shadow hover:bg-secondary transition-all active:scale-95 shrink-0"
            >
      ...
      <div className="container mx-auto px-4 sm:px-6 -mt-16 relative z-20">
        ...
        <div className="bg-card p-4 rounded-[24px] soft-shadow border border-border mb-12 flex items-center gap-4 transition-colors duration-300">
          ...
          <input 
            type="text" 
            placeholder="Cari nama surah atau nomor (misal: Al-Fatihah atau 36)..." 
          ...
          {filteredSurahs.map((surah) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground/30 group-hover:text-primary transition-colors">
                  {surah.city}
                </span>
              </div>
        ...
        {filteredSurahs.length === 0 && (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-bold text-foreground/40">Surah tidak ditemukan</h3>
            <p className="text-foreground/30">Coba gunakan kata kunci lain.</p>
          </div>
        )}
      </div>
    </main>
